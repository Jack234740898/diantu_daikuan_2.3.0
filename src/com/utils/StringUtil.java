...
public class StringUtil {
	...
	public static String getRandomFourDigit(boolean isPersonLoan){
		...
		if(h>=6&&h<=24){
			if(isPersonLoan){
				//2100到2500
				num = getFieldRandom(10000, 12037);
			}else{
				//1500到2000
			...
			if(isPersonLoan){
				//200到500
				num = getFieldRandom(5000, 10000);
			}else{
				//100到300
				num = getFieldRandom(100, 300);
			}
		}
		numStr = getFiveDigit(num);
		return numStr;
	}
	 ...
	 * 根据时间段对应比例  得到随机的四位数
	 */
	public static String getRandomFourDigit2(boolean isPersonLoan){
		Date date = new Date(System.currentTimeMillis());
		//Random r = new Random();
		//int rNum = r.nextInt(2500);
		int num = 15000;
		...
		HashMap<Integer,Float> maps = RandomData.getOnlineMap();
		if(maps.containsKey(h)){
			float rate = maps.get(h);
			num = (int)(num*rate);
			int min = (int)(num*(rate-0.05f));
			int max = (int)(num*(rate+0.05f));
			num = getFieldRandom(min, max);
		}
		return getFiveDigit(num);
	}
	...
	public static String getJSONObjectStr(JSONObject obj){
		if(null==obj)
			return "";
		LogUtil.d("getJSONArrayStr", "obj is "+obj.toString());
		...
		while (iterator.hasNext()) {
			...
			LogUtil.d("getJSONArrayStr", "bb is "+bb);
			sb.append(bb+"、");
		}
		if(sb.length()>0){
			sb = sb.deleteCharAt(sb.lastIndexOf("、"));
