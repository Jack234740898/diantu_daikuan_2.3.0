...
public class NotificationInitSampleHelper extends IMNotification{
	...
	private static final String TAG = "NotificationInitSampleHelper";
    private static boolean mNeedQuiet;
    private static boolean mNeedVibrator = true;
    private static boolean mNeedSound = true;
    private static MyApplication mContext;
    public NotificationInitSampleHelper(MyApplication context,Pointcut pointcut) {
        super(pointcut);
        this.mContext = context;
    }

    public void init(){
    	OpenImUtil openImUtil = mContext.mOpenImUtil;
    	LogUtil.d(TAG, "NotificationInitSampleHelper==openImUtil is "+openImUtil);
    	if(null==openImUtil)
    		return;
    	YWIMKit imKit = openImUtil.getIMKit();
		if (imKit != null){
			imKit.setEnableNotification(true);//开启通知栏提示
			String app_name = mContext.getResources().getString(R.string.app_name);
			imKit.setAppName(app_name+"");//通知栏显示的名称
			imKit.setResId(R.drawable.app_logo);//通知栏icon
...
//        }
    ...
    public Intent getCustomNotificationIntent(YWConversation conversation, YWMessage message, int totalUnReadCount) {
        ...
        //intent.setClass(mContext, FragmentTabs.class);
