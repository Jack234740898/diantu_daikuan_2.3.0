...
public class ConversationListUICustomSample extends IMConversationListUI {
	...
	public View getCustomConversationListTitle(final Fragment fragment,
		...
		leftImg.setImageResource(R.drawable.gray_back_down);
	...
	public View getCustomView(Context context, YWConversation conversation,
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(
					R.layout.demo_conversation_custom_view_item, parent, false);
		...
		viewHolder.conversationContent = (TextView) convertView
		...
		if (TextUtils.isEmpty(customViewConversation.getConversationName()))
			...
			viewHolder.conversationName.setText(customViewConversation
					.getConversationName());
		viewHolder.conversationContent.setText(customViewConversation
				.getLatestContent());
		return convertView;
	}
	 ...
	 */
	@Override
	public View getCustomItemView(Fragment fragment,
			YWConversation conversation, View convertView, int viewType,
		...
		if (viewType == type_1) {
			...
			if (convertView == null) {
				...
				convertView = inflater
						.inflate(R.layout.demo_custom_conversation_item_1,
								parent, false);
			...
			if (conversation.getConversationBody() instanceof YWCustomConversationBody) {
				...
				/*
				 * if (conversationId
				 * .equals(FragmentTabs.SYSTEM_TRIBE_CONVERSATION)) {
				 * headLoadHelper.setTribeDefaultHeadView(holder.head); name =
				 * "群系统消息"; } else if (conversationId
				 * .equals(FragmentTabs.SYSTEM_FRIEND_REQ_CONVERSATION)) {
				 * headLoadHelper.setDefaultHeadView(holder.head); name =
				 * "联系人系统消息"; } else {
				 * headLoadHelper.setDefaultHeadView(holder.head); name =
				 * "这是一个自定义会话"; }
				 */
			...
			if (convertView == null) {
				...
				convertView = inflater
						.inflate(R.layout.demo_custom_conversation_item_2,
								parent, false);
				holder.head = (ImageView) convertView.findViewById(R.id.head);
				holder.unread = (TextView) convertView
						.findViewById(R.id.unread);
				...
				holder.atMsgNotify = (TextView) convertView
						.findViewById(R.id.at_msg_notify);
			...
			YWTribeConversationBody body = (YWTribeConversationBody) conversation
					.getConversationBody();
			YWTribe tribe = body.getTribe();
			String name = tribe.getTribeName();
			holder.name.setText(name);
			YWIMKit imKit = LoginSampleHelper.getInstance().getIMKit();
			YWContactHeadLoadHelper helper = new YWContactHeadLoadHelper(
					fragment.getActivity(), null);
			/**
			 * !!!注:这里是给群回话设置头像,这里直接设置的默认头像 如果想自由设置可以使用
			 * {@link YWContactHeadLoadHelper#setCustomHeadView(ImageView, int, String)}
			...
			// 是否支持群@消息提醒
			boolean isAtEnalbe = YWAPI.getYWSDKGlobalConfig()
			...
			holder.time.setText(IMUtil.getFormatTime(conversation
					.getLatestTimeInMillisecond(), imKit.getIMCore()
					.getServerTime()));
			return convertView;
		}
		return super.getCustomItemView(fragment, conversation, convertView,
				viewType, headLoadHelper, parent);
	}
	...
	private void setSmilyContent(Context context, String content,
			ViewHolder2 holder) {
