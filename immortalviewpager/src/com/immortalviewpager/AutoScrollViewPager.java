...
public class AutoScrollViewPager extends ViewPager {
	...
	/**
	 * scroll factor for auto scroll animation, default is 1.0 设置自动滚动速率
	 **/
	private double autoScrollFactor = 6.0;
	/**
	 * scroll factor for swipe scroll animation, default is 1.0 设置手动滚动速率
	 **/
	...
	private ImmortalvpAdapter mAapter;

	@Override
	public void setAdapter(PagerAdapter adapter) {
		super.setAdapter(adapter);
		mAapter = (ImmortalvpAdapter) getAdapter();
		ChildCount = mAapter.getCount();
	}
	...
	// private OnPageScrolled scrolledListener ;
	// public void setOnPageScrolledListener(OnPageScrolled scrolledListener){
	...
	protected void onPageScrolled(int position, float offset, int offsetPixels) {
		...
		if (offset == 0 && ChildCount > 1) {
			if (position == ChildCount - 1) {
				setCurrentItem(1, false);
				mAapter.setCorrectPosition(0, ChildCount - 3);// lastPosition =
																// ChildCount -
																// 3
				lastPosition = 0;
			} else if (position == 0) {
				setCurrentItem(ChildCount - 2, false);
				mAapter.setCorrectPosition(ChildCount - 3, 0);// lastPosition
																			// =
																			// ChildCount
																			// -
																			// 3
				lastPosition = ChildCount - 3;
			} else {
				if (position - 1 == 0 && lastPosition == 0) {
					lastPosition = 1;
				}
				mAapter.setCorrectPosition(position - 1, lastPosition);
	...
	public void setScrollDurationFactor(int i) {
		...
		scroller.setScrollDurationFactor(i);
	}
